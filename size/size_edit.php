...
<div class="container">
  ...
  <?Php
  $id     = $_GET['id'];
  $query  = "SELECT * FROM tb_size WHERE id_size = '$id'";
  $data   = mysqli_query($koneksi, $query);
  $row    = mysqli_fetch_assoc($data);
  ?>

 <form action="edit_size_proses.php" method="post" enctype="multipart/form-data">
   <div class="mb-3 row">
     <label for="kd_size" class="col-sm-2 col-form-label">Size</label>
     <div class="col-sm-10">
       <input type="text" class="form-control" id="kd_size" name="kd_size"  value="<?= $row['kd_size']?>" required>
     </div>
   </div>
  <div class="mb-3 row">
    <label for="width" class="col-sm-2 col-form-label">Width</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="width" name="width"  value="<?= $row['width']?>" required>
    </div>
  </div>
  <div class="mb-3 row">
    <label for="height" class="col-sm-2 col-form-label">Height</label>
    <div class="col-sm-10">
      <input type="text" class="form-control" id="height" name="height"  value="<?= $row['height']?>" required>
    </div>
  </div>
    <div class="col-sm-10">
